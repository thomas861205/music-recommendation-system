...
def main():
    ...
    # conv2d:(ch_in, ch_out, k, stride, padding)
    # config = [
    #     ('conv2d', [32, 16, 3, 3, (2, 1), 0]),
    #     ('relu', [True]),
    #     ('bn', [32]),
    #     ('max_pool2d', [2, 2, 0]),
    #     ('conv2d', [64, 32, 3, 3, (2, 1), 0]),
    #     ('relu', [True]),
    #     ('bn', [64]),
    #     ('max_pool2d', [2, 2, 0]),
    ...
    mini = MusicDataset(root='', mode='train', n_way=2, k_shot=5, k_query=5, batchsz=10000, resize=-1)
    mini_test = MusicDataset(root='', mode='test', n_way=2, k_shot=5, k_query=5, batchsz=100, resize=-1)

    # db = DataLoader(mini, args.task_num, shuffle=True, num_workers=1, pin_memory=True)
    # for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db):
    #     x_spt, y_spt, x_qry, y_qry = x_spt.to(device), y_spt.to(device), x_qry.to(device), y_qry.to(device)
    #     output = maml.net(x_spt[0])
